...
public:
    virtual void draw() const;                 // deal with color and draw lines
...
struct Marked_polyline : Open_polyline {
    Marked_polyline(const string& m, int fsze = 15) :mark(m), fnt_sze{fsze} { }
    void draw_lines() const;
private:
    string mark;
    int fnt_sze;
...
struct Marks : Marked_polyline {
    Marks(const string& m, int f_sze = 15) :Marked_polyline(m, f_sze)
    {
...
struct Mark : Marks {
    Mark(Point xy, char c, int f_sze = 15) : Marks(string(1,c), f_sze)
    {
        add(xy);
    }
};
