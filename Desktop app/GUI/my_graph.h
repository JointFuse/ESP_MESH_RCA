...
/* Returns the coefficients of a simple linear equation based on two points on a line segment */
Poly::Simp_func_coef get_sfnc_coef(const int& x1, const int& y1, const int& x2, const int& y2);

/* Returns the point where two lines drawn by coefficients intersect */
Poly::Mpoint get_cross_point(const Point& p1, const Point& p2, const Point& p3, const Point& p4);

/* Checks if the point of intersection of two lines belongs to the given segments */
bool segments_overlap(const Point& p1, const Point& p2, const Point& p3, const Point& p4);

/* Checks if segments are parallel to each other */
bool parallel_segments(const Point& p1, const Point& p2, const Point& p3, const Point& p4);

/* Checks if a point lies in the rectangle formed by the diagonal between points p1 and p2 */
