...
void ip_check(string addr)
{
    ...
    case ISIP:
        return;
    case NOTIP:
        throw runtime_error("entered string is not an IP address");
    default:
        string mesg{ "typo when entering IP address:\n\n\t" };
        mesg += addr + "\n\t" + string(input_err, ' ') + '^';
...
try{
    ...
    {
    case SENDGOOD:
        info << "Key \'" << char(pressed) << "\' controls the device [MAC: " 
             << own->devices[dev_num].addr
             << " ] with duty cicle = " << value;
    ...
    case SENDFAIL:
        info << "Failed to communicate with the device [MAC: "
             << own->devices[dev_num].addr
             << " ] to confirm the setting of the control key \'"
             << char(pressed) << "\'.\nTry setting again";
        break;

    default:
        info << "Unknown error while configuring control key,"
             << " try again and contact the developer if necessary";
        break;
    }
...
try {
    ...
    {
    case SENDGOOD:
        info << "Duty cicle of the device [MAC: "
             << own->devices[dev_num].addr
             << " ] changed, new value = "
             << value;
    ...
    case SENDFAIL:
        info << "Failed to communicate with the device [MAC: " 
             << own->devices[dev_num].addr
             << " ] to confirm the duty cicle setting."
             << "\nTry setting again";
        break;

    default:
        info << "Unknown error while configuring duty cicle,"
             << " try again and contact the developer if necessary";
        break;
    }
...
try{
    ...
    info << "Address table received [ available devices: "
         << received_data.size() << " ] {\n";
...
ConnectHub::ConnectHub(Point xy)
    : Window(xy, width, height, "ESP MESH RCA v1.0"),
...
void ConnectHub::show_disconnected()
{
    ...
    b_stop.hide();
    b_connect.show();
    log_out("Ñommunication with devices lost");
    connected = false;
}
...
void ConnectHub::f_info()
{
    if (started) 
    {
        ...
        for (auto ptr = devices.begin(); ptr != devices.end(); ++ptr) 
        {
            info << "MAC: [ " << ptr->addr
                 << " ]; key: [ " << ptr->type
                 << " ] duty cicle = " << ptr->value << '\n';
        }

        if (!devices.size()) 
        { 
            info << "You haven't set up any device management yet,"
                 << " try pressing [info] to view available devices"; 
        }
    ...
    else 
    {
        ...
        info << "Connection instructions:\n"
             << "1)Download the \"Esptouch\" app to your smartphone\n"
             << "2)Connect smartphone to WiFi router and open Esptouch\n"
             << "3)Select the top menu item, in the window that opens, install Device count = 1 and Broadcast\n"
             << "4)If you have a password - enter it in the \"password\" field, enable ONE! device, "
             << "wait for a flashing red signal on device, click \"confirm\"\n"
             << "5)Wait for connection confirmation in Esptouch, enter the device IP in the desktop app\n"
             << "6)Green blinking signal of the device - confirmation of readiness for work, press \"connect\" in the desktop app\n"
             << "7)Turn on other devices and press confirm in Esptouch, green flashing signal - device "
...
{
    stringstream info;
    info << "Set the duty cicle with the slider and press the key,"
         << " which will control the device under the entered number\n";
    log_out(info.str());
...
int ConnectHub::_get_device_num()
{
    if (devices.size() == 0)
        throw runtime_error(
            "devices are not authorized, try pressing [info] for authorization"
        );
    ...
    if (num != -1) 
    {
        string info{ "the entered data is not a number\n\n\t" };
        info += check + "\n\t" + string(num, ' ') + "^";
    ...
    if (devices.size() < num || num < 1)
        throw runtime_error("Invalid device number entered");
